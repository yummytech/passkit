...
function fileIntoStream(stream, filename, callback) {
    var fileStream = fs_1.createReadStream(filename);
    stream.on('error', callback);
    stream.on('close', callback);
...
function pipeIntoStream(stream, source, callback) {
    ...
    if (typeof source === 'function')
        return source(stream, callback);
    throw new Error("Cannot load image " + source + ", must be String (filename), Buffer or function");
}
exports.pipeIntoStream = pipeIntoStream;
