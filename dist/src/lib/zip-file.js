...
var File = /** @class */ (function (_super) {
    ...
    function File(zip, filename, modified) {
        ...
        _this.modified = modified || new Date();
        ...
        _this._wroteHeader = false;
        // True if file fully written out; distinct from fully read (writable = false)
        _this._done = false;
        // Offset of file within the stream
        _this._offset = 0;
        _this._crc = 0 ^ -1;
        _this._uncompressedLength = 0;
        _this._compressedLength = 0;
        // Write to _deflate, count data size before passing to output.
        _this._deflate = zlib_1.createDeflateRaw();
        _this._deflate.on('data', function (buffer) {
            _this._compressedLength += buffer.length;
            zip._writeBuffer(buffer);
        });
        _this._deflate.on('end', function (buffer) {
            _this._doneWritingFile();
        });
    ...
    File.prototype.write = function (buffer, encoding) {
        ...
        var buf = typeof buffer === 'string' || buffer instanceof String
            ? Buffer.from(buffer, encoding)
            : buffer;
        // crc-32
        for (var i = 0; i < buf.length; i++) {
            var offset = (this._crc ^ buf[i]) & 0xff;
    ...
    File.prototype._writeLocalFileHeader = function () {
        ...
        var filename = Buffer.from(this.filename, 'utf-8');
        var buffer = Buffer.alloc(30 + filename.length);
        ...
        // last mod file time
        buffer.writeUInt16LE(get_part_time_1.getTimePart(this.modified), 10);
        // last mod file date
        buffer.writeUInt16LE(get_part_time_1.getDatePart(this.modified), 12);
        // crc-32
